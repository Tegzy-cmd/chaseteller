<?php 
    ...
    set_headers("Add New Deposit", "deposits");
    include("header.php");
    ...
    if (isset($_POST["add_deposit"])) 
    {
        ...
        $deposit_date = mysqli_real_escape_string($link, $_POST["deposit_date"]);
        $type = "deposit";
        ...
        elseif (empty($narration)) {
            ...
            // Fetch the transaction fee for deposit. 
            $fee_row = mysqli_fetch_array(mysqli_query($link, "SELECT `fee` FROM `transaction_fee` WHERE `level`='$type'"));
            $fee = $fee_row["fee"];

            // Insert a new transaction log to the transaction table.
            $transaction_statement = "INSERT INTO `transactions` (`user_id`, `trnx_id`, `amount`, `currency`, `fee`, `type`, `description`, `created_at`) ".
                                     "VALUES ('$user', '$trnx_id', '$amount', '$currency', '$fee', '$type', '$narration', '$deposit_date')";
            $insert_transaction = mysqli_query($link, $transaction_statement);

            // Insert the new deposit to the deposit table.
            $deposit_statement = "INSERT INTO `deposits` (`user_id`, `trnx_id`, `bank`, `account_name`, `account_no`, `amount`, `currency`, `charge`, `method`, `narration`, `created_at`) ".
                                 "VALUES ('$user', '$trnx_id', '$bank', '$account_name', '$account_no', '$amount', '$currency', '$fee', '$method', '$narration', '$deposit_date')";
            ...
            if ($insert_deposit && $insert_transaction) {
                $_SESSION["success"] = "New deposit record added successfully.";
                relocate_url("deposits.php"); 
            } 
            else {
                $_SESSION["error"] = "unable to add a new deposit record.";
            }
        }
    }
?>
                                ...
                                <div class="card-body">
                                    <form action="" method="post" class="form form-horizontal">
                                        <div class="form-body">
                                            ...
                                            <div class="form-group row">
                                                <label for="depositDate" class="col-md-3 label-control">Date</label>
                                                <div class="col-md-9">
                                                    <input type="datetime-local" id="depositDate" class="form-control" name="deposit_date">
                                                </div>
                                            </div>
                                        </div>
                                        <div class="form-actions center">
                                            <button type="submit" class="btn btn-success" name="add_deposit">Add Deposit</button>
                                        </div>
                                    </form> 
                                </div>
                            </div>
